...
def plot_spectype_breakdown(df):
    ...
    bars_r = ax.bar(x - width/2, retained_counts, width, label='Retained', color='#2ca02c', alpha=0.85)
    ...
    for bar, exc, ret in zip(bars_e, excluded_counts, retained_counts):
        ...
        if total > 0:
            ...
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 1000,
                    f'{pct:.0f}%', ha='center', va='bottom', fontsize=8, color='#d62728', fontweight='bold')
    ax.set_xticks(x)
...
def plot_synthetic_vs_empirical():
    ...
    ax.axhline(65.3, color='#1f77b4', linestyle='--', linewidth=1.2, alpha=0.6)
...
def plot_skymap(df):
    ...
    legend_elements = [
        Line2D([0],[0], marker='o', color='w', markerfacecolor='#d62728',
               markersize=10, label=f'Excluded (N = 1,145,218 | 65.3%)', linewidth=0),
        Line2D([0],[0], marker='o', color='w', markerfacecolor='#1f77b4',
               markersize=10, label=f'Retained (N = 608,917 | 34.7%)', linewidth=0),
    ]
    ax.legend(handles=legend_elements, loc='lower right',
              facecolor='white', edgecolor='black', fontsize=11)
    ax.set_title('Sky Distribution of SETI Candidate Stars — Gaia DR3\n'
                 'Parametric Avoidance Model  |  N = 1,754,135',
